...
\begin{lemma}
    ...
    \dots, n_k \) be positive integers with greatest common divisor \( d
    \).  Then there exists a positive integer \( M \) such that if \( m
    \ge M \) then there exist nonnegative integers \( c_1, c_2, \dots, c_k
    \) such that
    \[
        md = \sum\limits_{j=1}^k c_j n_j.
    \]
\end{lemma}
\begin{proof}
    \begin{enumerate}
        \item
            Let
            \[
                A = \setof{n}{n=c_1n_1 + \cdots + c_k n_k, c_k \text{%
                positive integers}}
            \] and
            \[
                B = \setof{b_1 m_1 + \cdots + b_j m_j}{ m_i \in A , b_i
                \in \Integers, i=1,\dots, j}.
            \]
        \item
            Let \( d' \) be the least positive integer in \( B \), where
        ...
        \item
            ...
            + \cdots + c_k n_k \in A \).  By the Division Algorithm let \(
            a = d'q + r \) with \( 0 \le r < d' \).  Then
            \begin{align*}
                r &= a - d' q \\
                &= a - q (b_1 m'_1 + \cdots + b_j m'_j) \\
                &= (c_1 n_1 + \cdots + c_k n_k) - q \left(\sum\limits_{i=1}^j
                b_i\sum\limits_{\ell=1}^k c'_{\ell i} n_l \right) \\
                &= \left( c_1 - q \sum\limits_{i=1}^j b_i c_{\ell i}
                \right) n_1 + \cdots + \left( c_k - q \sum\limits_{i=1}^j
                b_i c_{\ell i} \right) n_k.
            \end{align*}
        \item
            This means that \( r \in B \).  Since \( 0 \le r < d' \),
            and \( d' \) is the least positive integer in \( B \), then \(
            r = 0 \) and \( d' \) is a divisor of \( a \), establishing
            the first claim.
        \item
            For simplicity \( d' = d = \gcd(n_1, \dots, n_k) \).
            Rearrange the terms in the representation of \( d = b_1 m_1
            + \cdots + b_j m_j = a_1 n_1 + \cdots + a_k n_k \) so that
            ...
            Algorithm \( K = q' N_2/d + b \) with \( 0 \le b < N_2/d \)
            and the integer \( q' = \lfloor (N_2/d)/k \rfloor \).  Now \(
            md = N_2^2 + (q' N_2/d + b) d = N_2(N_2 + j -b ) + b N_1 \).
        \item
            Since \( N_2 \in A \) and \( N_1 \in A \), then there exist
            nonnegative integers \( c_1, c_2, \dots, c_k \) such that
            \[
                md = \sum\limits_{j=1}^k c_j n_j.
            \]
    \end{enumerate}
        ...
        \item
            \begin{enumerate}
                \item
                    ...
                    communicating states be labeled \( i = 1, \dots, k \).
...
\begin{definition}
    The \defn{expected return time} is \( \E{\tau_{ii}} = m_i \sum\_{\nu=0}^
    {\infty} \nu f^\nu_{ii} \).
\end{definition}
...
\begin{proposition}
    \begin{enumerate}
        \item
            ...
            \[
                \sum\limits_{n=1}^{\infty} (P^n)_{ii} = \infty.
            \]
        \item
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
            ...
            \[
                N_i = \sum\limits_{j=1}^{\infty} \indicatorrv{X_j=i}.
            \]
        \item
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
            ...
            0 \).
        \item
            \label{defA} With \( x_0 = j \), let \( A = \set{x_l \ne j,
            1 \le l < n, x_n = i} \) be the path of states visited
            before \( i \).  Note that \( \Prob{A} > 0 \).
        \item
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
            ...
            \mathcal{X} \).
            \[
                \lim_{n \to \infty} \frac{1}{n} \sum\limits_{m=1}^n (P^m)_
                {ij} = 0.
            \]
        \item
            Summing over \( j \)
            \[
                ...
                \frac{1}{n} \sum\limits_{m=1}^{n} (P^m)_{ij} = 0.
            \]
        \item
        ...
        \item
            ...
            \[
                \lim_{n \to \infty} \frac{1}{n} \sum\limits_{m=1}^{n}
                \sum\limits_{j \in \mathcal{X}} (P^m)_{ij} = 0
...
\begin{definition}
    The \defn{expected return time} \( m_i \) is \( \sum_{\nu=1}^\infty
    \nu f^\nu_{ii} \).
\end{definition}
...
\begin{remark}
    Note that this theorem says that a probability transition matrix for
...
The first step defines the states and transition probabilities.  We
might think that because the bathroom can be either \emph{occupied} or
\emph{vacant}, and the sign in front can either read ``Vacant'' or
``Occupied'', that there should be four states, one for each possible
pair of occupation state and sign.  However, this is not the case.
Consider the state ``bathroom is occupied and the sign says it’s
occupied''.  We must distinguish between the cases where the person
occupying the bathroom is conscientious (they will definitely slide the
sign to ``Vacant'' when they leave) or not (they will leave the sign as
``Occupied'' after they leave).

We must therefore add more states to our Markov chain that correspond to
...
\begin{codebox}
    \Procname{Bathroom Markov Chain Simulation} \zi Comment Post:  Example
...
\begin{exercise}
    ...
    empty, remove a ball from that urn and put it in the other urn. Make
        ...
        \item
            ...
            \begin{enumerate}
                ...
                \item
                  Then
                  \begin{multline*}
                     \sum_{n=1}^{\infty}(P^n)_{jj} > \sum_{n=1}^{\infty}
